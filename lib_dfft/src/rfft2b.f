...
C     * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
C
      SUBROUTINE DRFFT2B (LDIM, L, M, R, WSAVE, LENSAV, WORK,
     1  LENWRK, IER)
...
C
      ...
      R(1,J) = -R(1,J)
      END DO
      CALL DRFFTMB(1,1,M,LDIM,R,M*LDIM,
...
C
      ...
      CALL DCFFTMB(LDH-1,1,M,LDH,WORK(2),LDH*M,
...
C
      ...
      R(L,J) = -R(L,J)
      END DO
      CALL DRFFTMB(1,1,M,LDIM,R(L,1),M*LDIM,
     1     WSAVE(LWSAV+MWSAV+1),MMSAV,WORK,LENWRK,IER1)
      END IF
C
...
C
      ...
      END DO
      END DO
      CALL DRFFTMB(M,LDIM,L,1,R,M*LDIM,WSAVE(1),
